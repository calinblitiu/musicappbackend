                ...
                <div class="box">
                    ...
                    <div class="box-body table-responsive no-padding">
                        <table class="table table-hover">
                            ...
                            <?php for($i = 1; $i<=5; $i++): ?>
                                <tr data-item-id="<?=$sample['key_item_'.$i][0]['id']?>" data-key-no="<?=$i?>">
                                    <th><?=$i?></th>
                                    <th class="sync4-cell-name" style="cursor: pointer;text-decoration: underline;"><?=$sample['key_item_'.$i][0]['name']?></th>
                                    <th data-key="1">
                        ...
                        <form action="<?=base_url()?>index.php/sync4-upload-music" role="form" enctype='multipart/form-data' id='sync4-upload-form' method="post" style="display: none;">
                            <input type="hidden" name="sync4-cell-id" id="sync4-cell-id" value="">
                            <input type="hidden" name="sync4-music-no" id="sync4-music-no">
                            <input type="hidden" name="sync4-id" value="<?=$sample['id']?>">
                            <input type="hidden" name="sync4-cell-no" id="sync4-cell-no">
        ...
        <form action="<?=base_url()?>index.php/deletesync4musicfile" method="post">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <input type="hidden" name="sync4-cell-id" id="sync4-cell-del-id">
                    <input type="hidden" name="sync4-cell-no" id="sync4-cell-del-no">
                    <input type="hidden" name="sync4-id" value="<?=$sample['id']?>">
...
<div id="editmodal" class="modal fade" role="dialog">
    <div class="modal-dialog">
        <form action="<?=base_url()?>index.php/editsync4name" method="post">
            <!-- Modal content-->
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4 class="modal-title">Edit Sync4 Name</h4>
                </div>
                <div class="modal-body">
                    <input type="hidden" name="sync4-cell-id" id="sync4-cell-edit-id">
                    <input class="form-control" type="text" name="sync4-cell-name" id="sync4-cell-edit-val" required="" placeholder="Please Input Sync4 Name">
                    <input type="hidden" name="sync4-id" value="<?=$sample['id']?>">
                </div>
                <div class="modal-footer">
...
<script type="text/javascript">
    ...
    $('.listen-music-btn').click(function(){
        var me = $(this);
        var music_url = baseURL+"assets/sync4-musicfiles/"+me.data('music-url')+"?"+makeid();
    ...
    $(".edit-music-btn").click(function(){
        ...
        $("#sync4-cell-id").val(cell_id);
        $("#sync4-music-no").val(cell_item_no);
        $("#sync4-cell-no").val(cell_no);
    ...
    $(".remove-music-btn").click(function(){
        ...
        $("#sync4-cell-del-id").val(cell_id);
        $("#sync4-cell-del-no").val(cell_item_no);
    ...
    $(".sync4-cell-name").click(function(){
        ...
        // alert(cell_id);
        $("#sync4-cell-edit-id").val(cell_id);
        $("#sync4-cell-edit-val").val(me.html());
